...
def addAerosol(atmosphere, X, tau0=0.05, alpha=1.0, wavelen0=440.0, plotAtmosphere=True):
    ...
    atmosphere_aerosol.set_bandpass(wavelen=atmosphere.wavelen, sb=atmosphere.sb * np.exp(-tau * X))

    if plotAtmosphere:
        # Plot for a check:
        atmodict = {"Original atmosphere": atmosphere, "With aerosols": atmosphere_aerosol}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Add a standard aerosol component to an atmosphere without aerosols"
    )
    parser.add_argument(
        "atmosphereFile", type=str, default=None, help="The atmosphere file to add aerosols to."
    )
