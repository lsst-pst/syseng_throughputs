...
if __name__ == "__main__":
    ...
    hardware, system = bu.buildHardwareAndSystem(
        defaultDirs, addLosses=addLosses, atmosphereOverride=atmos_std
    )
    ...
    for f in filters:
        filters[f].write_throughput(
            os.path.join(outDir, "filter_%s.dat" % (f)), print_header=header + perfilterheader[f]
        )
        hardware[f].write_throughput(
            os.path.join(outDir, "hardware_%s.dat" % (f)), print_header=header + perfilterheader[f]
        )
        system[f].write_throughput(
            os.path.join(outDir, "total_%s.dat" % (f)), print_header=systemheader + perfilterheader[f]
        )
