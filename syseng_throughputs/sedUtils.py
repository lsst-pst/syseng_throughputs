...
def readPhotSeds(sedDir=None):
    ...
    sedlists["galaxies"] = [
        "Sa_template_norm.sed.dat",
        "Sdm_template_norm.sed0.dat",
        "Ell2_template_norm.sed.dat",
    ]
    sedlists["photoZ_outliers"] = [
        "xspec_172.sed.dat",
        "xspec_173.sed.dat",
        "xspec_175.sed.dat",
        "xspec_176.sed.dat",
...
def matchSedsBp(sedDict, bpDict, refFilter=None):
    ...
    for f in bpDict.keys():
        if np.any(bpDict[f].wavelen != wavelen_match):
            bpDict[f].resampleBandpass(
                wavelen_min=wavelen_match.min(),
                wavelen_max=wavelen_match.max(),
...
def plotDmagsSingle(
    ...
    plt.legend(handles=handles, loc=(0.9, 0.2), fontsize="small", numpoints=1, fancybox=True)
    if savefig:
        if titletext != None:
            plt.savefig("%s.%s" % (titletext, figformat), format=figformat)
        else:
